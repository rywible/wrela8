    ...
    fn each_release_opt_is_re_asked_alone_on_the_product_tier() {
        ...
        for opt in RELEASE_OPTS {
            ...
            let base: &[OptId] = match opt {
                // Decision 1791 widened this from `[NarrowImm]`. C5 cannot
                // be ranked against `dev` at all (1747), and on the product
                // tier it cannot be ranked without `RegAlloc` either: its
                // saving is words, and words only become cycles once the
                // allocator has removed the schedule slack that absorbs
                // them. Both halves of that are measured in
                // plans/codegen-pareto-C.md; the membership claim itself is
                // pinned by leave-one-out, which has no baseline freedom.
                OptId::WideImmForms => &[OptId::NarrowImm, OptId::RegAlloc],
                _ => &[],
            };
