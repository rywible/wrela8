...
fn check_device_state_call(
    ...
    match method {
        ...
        // plans/M7.md item G (03-hardware.md §6): an `IrqCap[V]` out of the
        // claim. The runtime word is the vector bit index (decision 12);
        // whether this driver *owns* a vector is checked over the sealed
        // image graph (`eval::image_checks::check_vector_bindings`), not
        // here — a body can type-check a `take_irq` against a device that
        // declared none, and the graph pass is what names that unowned.
        "take_irq" => {
            if !args.is_empty() {
                return Err(type_error(
                    format!(
...
pub fn is_device_transport_intrinsic(key: &str) -> bool {
    matches!(
        key,
        "Device.claim" | "Device.map_partition" | "Device.take_irq"
    )
}
