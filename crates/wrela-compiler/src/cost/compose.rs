...
"#,
    ...
    fn a_case_with_no_block_vector_is_untouched_by_the_wiring() {
        ...
        // The pin moves with the release form it pins. `cost-branchy`'s
        ...
        // (134), and **109 once item F's whole-program convention did**
        // — a 19 % fall on one case from the frames F3 deletes and the
        // reloads F1/F2 keep in registers across calls. The number is
        // pinned so the *wiring* stays inert, not to pin a cost model —
        // re-measure it, never rescale it.
        assert_eq!(report.total_proxy_cycles, 109, "the pinned flat total");
        assert_eq!(report.workload_totals["flat"], 109);
