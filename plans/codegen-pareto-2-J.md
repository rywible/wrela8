...
| **1931** | The inliner never builds a frame past the 4 095-byte `imm12` window `codegen::build_frame` fails closed on, computed conservatively (spill-everything) because it runs before `RegAlloc`. Found by `cost-icache-cliff`, which stopped compiling without it. *Superseded by 1935.* |
