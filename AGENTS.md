...
- `cargo xtask fuzz [lexer|parser|sema|eval|lower|async|imports]
  ...
  engine. Every lane checks panics / nondeterminism / category set /
  `internal error:`, and every lane's summary prints its **measured
  reach** into the pass it exists to test (plans/M9.md item PP).
